import { getCommandLineArgs, Logger, Execute } from '@/libs'
...
export default async function integration(): Promise<void> {
  ...
  if (commandLineParams['--parallel']) {
    Execute.tasks(tasks, 'parallel')
      .then(({ results, errors }) => {
        Logger.info('Risultati in parallelo:', results) // [1, null, true]
        Logger.err('Errori in parallelo:', errors) // [null, Error: "Errore nel task 2", null]
      })
      .catch((error) => {
